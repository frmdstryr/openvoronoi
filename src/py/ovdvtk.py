...
class VD:
    ...
    def setVDText2(self, times):
        ...
        self.vdtext2_text += "= {0:.2f} us*N*log2(N) \n".format(
            1e6 * float(times[0]) / (float(pts) * float(math.log10(pts) / math.log10(2.0))))
        self.vdtext2_text += "M={0} line-sites in {1:.3f} s ".format(lns, times[1])
        self.vdtext2_text += "= {0:.2f} us*M*log2(M)".format(
            1e6 * float(times[1]) / (float(lns) * float(math.log10(lns) / math.log10(2.0))))
    ...
    def setGenerators(self):
        ...
        for pt_data in self.vd.getGenerators():
            ...
            if self.drawVertexIndex:
                id_text = str(idx)
                factor = FollowerText(text=id_text, center=(p.x, p.y, 0), scale=self.textScale,
                                      color=self.pointsiteColor)
                factor = FollowerText(text=id_text, center=(p.x, p.y, 0), scale=self.textScale,
                                      color=self.pointsiteColor)
    ...
    def setEdgesPolydata(self):
        ...
        linePolyData.GetPointData().Update()
...
class Tube(CamvtkActor):
    ...
    def __init__(self, p1=(0, 0, 0), p2=(1, 1, 1), radius=0.1, color=(0, 1, 1)):
        ...
        self.mapper = vtk.vtkPolyDataMapper()
        self.mapper.SetInputData(self.tubefilter.GetOutput())
        self.SetMapper(self.mapper)
        self.SetColor(color)
