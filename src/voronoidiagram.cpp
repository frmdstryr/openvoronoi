...
bool VoronoiDiagram::insert_line_site(int idx1, int idx2, int step) {
    ...
    std::tie(start,end) = find_endpoints(idx1,idx2);
    g[start].status=OUT;
    g[end].status=OUT;
    g[start].zero_dist();
    g[end].zero_dist();
    ...
    std::tie(seg_start, start_null_face, pos_sep_start, neg_sep_start, start_to_null) = find_null_face(start, end  , left, dir1, pos_site);
    std::tie(seg_end  , end_null_face  , pos_sep_end  , neg_sep_end  , end_to_null  ) = find_null_face(end  , start, left, dir2, pos_site);
    ...
    HEEdge pos_edge, neg_edge;
    {
        // 2012-03-20: coverage-testing shows this never happens!
        //if ( linesite_k_sign ) {
        //    std::tie(pos_edge, neg_edge) = g.add_twin_edges( seg_start,   seg_end );
        //    g[pos_edge].inserted_direction = true;
        //    g[neg_edge].inserted_direction = false;
        //} else {
        assert(!linesite_k_sign);
            std::tie( pos_edge, neg_edge) = g.add_twin_edges( seg_end  ,seg_start );
    ...
    { // add SEPARATORS
        ...
        typedef std::tuple<HEEdge, HEVertex, HEEdge,bool> SepTarget;
...
void VoronoiDiagram::insert_arc_site(int idx1, int idx2, const Point& center, bool cw, int step) {
    ...
    HEEdge pos_edge, neg_edge;
    {
        if (cw)
            ...
            std::tie( neg_edge, pos_edge) = g.add_twin_edges( seg_end  ,seg_start );
...
std::tuple<HEVertex,HEFace,HEVertex,HEVertex,HEFace>
VoronoiDiagram::find_null_face(HEVertex start, HEVertex other, Point left, Point dir, Site* new_site) {
    ...
    if (g[start].null_face != g.HFace() ) { // there is an existing null face
        ...
        std::tie(next_edge,prev_edge) = g.find_next_prev(start_null_face, seg_start);
        //assert( g[prev_edge].next == next_edge );
        std::tie( neg_sep_start, face_to_null ) = process_null_edge(dir, next_edge, k3_sign, true);
        std::tie( pos_sep_start, face_to_null ) = process_null_edge(dir, prev_edge, k3_sign, false);
        if (debug) {
            //std::cout << "find_null_face() endp= " << g[start].index << " has existing null_face : " << g[start].null_face << "\n";
            g.print_face( g[start].null_face  );
        }
        return std::make_tuple( seg_start, start_null_face, pos_sep_start, neg_sep_start, face_to_null);
        ...
        HEEdge e1,e1_tw;
        std::tie(e1,e1_tw) = g.add_twin_edges(seg_start,pos_sep_start);
        HEEdge e2,e2_tw;
        std::tie(e2,e2_tw) = g.add_twin_edges(pos_sep_start,neg_sep_start);
        HEEdge e3,e3_tw;
        std::tie(e3,e3_tw) = g.add_twin_edges(neg_sep_start,seg_start);
...
void VoronoiDiagram::add_separator(HEFace f, HEFace null_face,
                                   std::tuple<HEEdge, HEVertex, HEEdge, bool> target,
    ...
    std::tie( endp_next_tw, endp_prev_tw ) = g.find_next_prev(null_face, sep_endp);
    if (debug) {
    ...
    HEEdge v_previous = std::get<0>(target);
    HEVertex v_target = std::get<1>(target);
    HEEdge    v_next  = std::get<2>(target);
    bool    out_new_in = std::get<3>(target);
    if (debug) {
    ...
    HEEdge e2, e2_tw;
    std::tie(e2,e2_tw) = g.add_twin_edges( sep_endp, v_target );
    g[e2].type    = SEPARATOR;
...
void VoronoiDiagram::add_edge(EdgeData ed, HEFace newface, HEFace newface2) {
    ...
    if ( src_sign == trg_sign ) {  // add a single src-trg edge
        ...
        HEEdge e_new, e_twin;
        std::tie(e_new,e_twin) = g.add_twin_edges( new_source, new_target );
        ...
        */
        ...
        HEEdge e1, e1_tw;
        HEEdge e2, e2_tw;
        std::tie(e1, e1_tw) = g.add_twin_edges( new_source, apex );
        std::tie(e2, e2_tw) = g.add_twin_edges( apex, new_target );
...
///
std::tuple<HEEdge,HEVertex,HEEdge,bool> VoronoiDiagram::find_separator_target(HEFace f, HEVertex endp) {
    if (endp==HEVertex()) // no separator
        return std::make_tuple( HEEdge(), HEVertex(), HEEdge(), false) ;
    ...
    return std::make_tuple(v_previous, v_target, v_next, flag);
}
